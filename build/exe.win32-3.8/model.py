...
class CardCollection:
    ...
    def __init__(self, cards_list=[], should_draw = False, name = "", label = "", droppable=False, dimensions=[CARD_WIDTH, CARD_HEIGHT], hide_spot_marker=False):
        self.clickable = True
        ...
        if (should_draw):
            all_card_collections.append(self)
        self.sprite = None
        self.droppable = False
        if (self.label != ""):
            self.sprite = pygame.sprite.Sprite()
            self.sprite.image = pygame.image.load(os.path.join(img_folder, "spr_card_outline.png"))
            self.sprite.image = NineSlice.get_nine(self.sprite.image, (8, 8, 8, 8), self.dimensions[0], self.dimensions[1])
            text_render = CardCollection.font.render(self.label, True, WHITE)
            text_scale_w = CARD_WIDTH - 2 * padding
...
class Deck:
    def __init__(self):
        ...
        self.deck.x = 0
        self.deck.y = 0
        self.deck.set_position(PLAY_WIDTH / 2 - (CARD_WIDTH + padding) / 2, HEIGHT / 2)
    ...
    def cut(self, i):
        if self.cut_state == "ready_to_cut":
            ...
            for card in _cards[:i + 1]:
                ...
                c_spr.set_move_target(c_spr.rect.x - (CARD_WIDTH + padding), c_spr.rect.y)
...
class Hand:
    def __init__(self, cards_list=[], should_draw=False, name = "", label = "", droppable=False, dimensions=[CARD_WIDTH, CARD_HEIGHT]):
        ...
        self.hand.x = random.randint(50, PLAY_WIDTH)
        self.hand.y = random.randint(50, HEIGHT)
